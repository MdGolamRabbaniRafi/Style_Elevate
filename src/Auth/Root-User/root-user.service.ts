...
export class RootUserService {
  ...
  async SearchByID(Id: number): Promise<RootUserEntity | null> {
  ...
  async EditUserProfileByID(
    Id: number,
    updatedData: Partial<RootUserEntity>,
  ...
  async ChangeProfilePicture(
    Id: number,
    path: string,
  ): Promise<RootUserEntity | { message: string }> {
  ...
  // In UserService
  async getAllUsers(): Promise<RootUserEntity[] | null> {
  ...
  async findByEmail(email: string): Promise<RootUserEntity | null> {
  ...
  async validate(email: string, password: string): Promise<RootUserEntity | null> {
    // console.log("email:"+email)
  ...
  async newPassword(newPassword: string, Id: number): Promise<any> {
    const findUser = await this.SearchByID(Id);
  ...
  // async SignUp(RootUserEntity: RootUserEntity): Promise<RootUserEntity | boolean> {
  //   try {
  //     const saltRounds = 10;
